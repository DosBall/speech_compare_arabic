...
@app.route('/answer', methods=['GET', 'POST'])
def answer():
    file1 = request.files.get('audio')
    file_path = "api_storage/" + file1.filename
    file1.save(file_path)
    letter_num = request.form.get('letter')
    k = request.form.get('k')
    if k is not None:
        k = int(str(k))
    if letter_num is not None:
        letter_num = int(str(letter_num))
    ans, j, p = test_models1(file_path, letter_num, k)
